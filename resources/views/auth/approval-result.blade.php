...
@section('content')
    <div class="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div class="bg-white rounded-2xl shadow-lg w-full max-w-lg p-8">
            ...
            @if (($status ?? '') === 'success') bg-green-50 border-green-200 text-green-800
            @elseif(($status ?? '') === 'warning')
                bg-red-50 border-red-200 text-red-800
            @else
                bg-gray-50 border-gray-200 text-gray-800 @endif">
                {{ $message ?? '' }}
            </div>
            @if (($status ?? '') === 'success')
                <div class="mt-6 text-sm text-gray-600">
                    You can now close this page. The user may open the app on any device (desktop or mobile) and log in
